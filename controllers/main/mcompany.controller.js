...
app.get('/',mainAuth, async (req, res, next) => {
    ...
    try {
        ...
        return res.render('mainAdmin/company/companyListing.ejs');
...
app.post('/status',mainAuth,async(req,res,next) => { 
    ...
    if(params.status=="0")  status=1
       ...
       if(updatedResponse)
           ...
           return responseHelper.post(res,  params.status==0?"Admin "+appstrings.active_success:"Admin "+appstrings.inactive_success,updatedResponse);
             }
             else{
...
app.get('/add',mainAuth, async (req, res, next) => {
    
  try{
  
    return res.render('mainAdmin/company/addCompany.ejs',);
...
app.get('/compare',mainAuth, async (req, res, next) => {
    
  try{
  
    return res.render('mainAdmin/company/compare.ejs',);
...
app.get('/permissions',mainAuth, async (req, res, next) => {
  ...
  try{
    ...
    return res.render('mainAdmin/company/permissions.ejs',{permissions});
...
app.post('/permissions',mainAuth,async (req, res) => {
  try {
...
for(var k=0;k<services.rows.length;k++)
{
  var services2 = await COMPANY.findAll({
    where: {parentId:services.rows[k].id,role:2},
 })
 services.rows[k]['totalVendors']=services2.length
}
...
app.post('/update',mainAuth,async (req, res) => {
  try {
    const data = req.body;
    var logo1 = "";
    var logo2 = "";
...
app.get('/view/:id',mainAuth,async(req,res,next) => { 
  ...
  return res.redirect(mainpath+"company");
}
      ...
      return res.render('mainAdmin/company/viewCompany.ejs',{data:findData,chargesData});
