...
app.get('/getprofile',mainAuth,async(req,res,next) => {
    try {
        const findData = await COMPANY.findOne({
          where :{id :req.companyId }
        });
        return res.render('mainAdmin/dashboard/profile.ejs',{data:findData});
      } catch (e) {
        req.flash('errorMessage',e.message)
        return res.redirect(mainpath);
      }
  });
  app.post('/companyUpdate',mainAuth,async (req, res) => {
    try {
      const data = req.body;
      console.log(data);
      var logo1 = "";
