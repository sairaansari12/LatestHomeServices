  ...
  try {
    ...
    if (userData) {
      ...
      if (updatedResponse) {
        ...
        return responseHelper.post(res,   params.status==0?"Employee "+appstrings.active_success:"Employee "+appstrings.inactive_success, updatedResponse);
      }
      else {
...
app.post('/update', adminAuth, async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      var userdata=await EMPLOYEE.findOne({where:{phoneNumber:data.phoneNumber,
        id: {
          [Op.not]:data.empId
        }
