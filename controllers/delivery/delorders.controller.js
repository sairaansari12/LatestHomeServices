     ...
     if(updatedResponse)
           {
            ...
            var notifPushUserData={title:"Job completed ,Order No.- "+userData.orderNo,
            description:"Job with order No.- "+userData.orderNo +' is completed by '+ req.userData.firstName+' on  ' +commonMethods.formatAMPM(new Date),
               ...
               var findData=await USER.findOne({where:{id:userData.userId}});
                var notifPushUserData={title:"Order No.- " + userData.orderNo + ' status is marked ' + statusName,
...
var jobStatus = await ORDERS.findOne({attributes:['id'],where:{progressStatus:['8','9','10']},include: [
  {model: ASSIGNMENT , attributes: ['id','jobStatus'], required:true, 
       ...
       if(updatedResponse)
             {
                   ...
                   var notifPushUserData={title:"Assignment Rejected for  order No.- "+userData.orderNo +' on ' +commonMethods.formatAMPM(new Date),
...
if(assignData && assignData.dataValues)
{
  await ASSIGNMENT.destroy({
   ...
   var notifPushUserData={title:"Job Accepted ,Order No.- "+userData.orderNo,
